...
class DataLoaderImpl(DataLoader):
    ...
    def __init__(self, data_config, data_loader, action_horizon: int = 30):
        self._data_config = data_config
        self._data_loader = data_loader
    ...
    def __iter__(self):
        ...
        for batch in self._data_loader:
            ...
            # BehaviorLeRobotDataset returns 1-step actions (batch, dim).
            # Expand to (batch, action_horizon, dim) by repeating across time.
            if hasattr(actions, 'ndim') and actions.ndim == 2:
                actions = jnp.repeat(actions[:, None, :], self._action_horizon, axis=1)
            yield obs, actions
...
def create_behavior_dataset(data_config: _config.DataConfig, action_horizon: int, seed: int | None = None) -> Dataset:
    ...
    tasks = data_config.task_names if data_config.task_names is not None else _all_tasks
    logging.info(f"Loading tasks: {tasks}")
...
def create_behavior_data_loader(
    ...
    *,
    ...
    return DataLoaderImpl(data_config, data_loader, action_horizon=config.model.action_horizon)
