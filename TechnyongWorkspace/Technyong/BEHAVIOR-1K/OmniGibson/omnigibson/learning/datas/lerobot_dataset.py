...
class BehaviorLeRobotDataset(LeRobotDataset):
    ...
    def __init__(
        ...
        self.root = Path("/home/data/Technyong_workspace/behavior-1k-solution/data/behavior_datasets")
        ...
        # 1. [핵심] 로컬 파티션(Parquet) 데이터를 실제로 불러오기
        from datasets import load_dataset
        import os
        import torch as th
        import numpy as np

        local_data_dir = "/home/data/Technyong_workspace/behavior-1k-solution/data/behavior_datasets/data"
        
        # 서버 접속 없이 내 컴퓨터의 모든 데이터를 로드
        self.hf_dataset = load_dataset(
            "parquet", 
            data_files=os.path.join(local_data_dir, "**/*.parquet"),
            split="train"
        )

        # 2. [정석 타겟팅] 사진에 있는 진짜 에피소드 번호를 넣습니다.
        # 00250020 -> 250020 (앞의 0은 제외)
        target_episodes = [250020, 250030, 250040, 250050] 
        
        all_episode_indices = np.array(self.hf_dataset["episode_index"])
        matched_indices = np.where(np.isin(all_episode_indices, target_episodes))[0]
        
        self.hf_dataset = self.hf_dataset.select(matched_indices)
        
        print(f"✅ 완벽 동기화 완료! 에피소드 {target_episodes}에서 {len(self.hf_dataset)} 프레임을 확보했습니다.")
       
        # 3. [에러 방지] 에피소드 주소록(Index) 재구성
        # 필터링 직후에 이 코드가 반드시 있어야 합니다.
        unique_episodes = np.unique(self.hf_dataset["episode_index"])
        self.episodes = unique_episodes.tolist()
        
        if len(self.episodes) == 0:
            print("❌ 여전히 0개입니다! '0025'라는 단어가 데이터 안에 없는 것 같습니다.")
            # 이 경우 실제 데이터셋의 컬럼 내용을 확인해야 합니다.
        self._num_episodes = len(self.episodes)
        
        # .item() 에러를 피하기 위한 인덱싱 계산
        ...
        # 4. [비디오 연결 확인]
        video_path = self.root / "videos"
        if not video_path.exists():
            print(f"⚠️ 경고: 비디오 폴더({video_path})를 찾을 수 없습니다!")
        else:
            print(f"DEBUG: Found {len(os.listdir(video_path))} tasks in video folder. Syncing...")
...
class BehaviorLerobotDatasetMetadata(LeRobotDatasetMetadata):
    ...
    def __init__(
        ...
        self.repo_id = repo_id
        self.revision = revision if revision else CODEBASE_VERSION
        # 기존 코드를 주석 처리하고 아래 경로로 강제 고정합니다.
        # self.root = Path(os.path.expanduser(str(root))) ... 
        #self.root = Path("/home/data/Technyong_workspace/behavior-1k-solution/data/behavior_datasets")
        # 기존 경로는 주석 처리하고 아래 한 줄을 정확히 입력하세요.
        self.root = Path("/home/data/Technyong_workspace/behavior-1k-solution/data/behavior_datasets")
        try:
            if force_cache_sync:
