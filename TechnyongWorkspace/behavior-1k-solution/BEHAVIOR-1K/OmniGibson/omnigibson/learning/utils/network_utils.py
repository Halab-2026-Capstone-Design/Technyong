    ...
    async def _handler(self, websocket):
        ...
        while True:
            try:
                ...
                # msgpack 0.5.x decodes keys as bytes by default — normalize to str
                if result and isinstance(next(iter(result)), bytes):
                    result = {k.decode("utf-8") if isinstance(k, bytes) else k: v for k, v in result.items()}
                if "reset" in result:
                    self._policy.reset()
                    continue
...
def unpack_array(obj):
    ...
    if ndarray_key in obj:
        ...
        return np.ndarray(buffer=obj[data_key], dtype=np.dtype(obj[dtype_key]), shape=obj[shape_key])
