import os, multiprocessing; os.cpu_count = lambda: 2; multiprocessing.cpu_count = lambda: 2
...
def process_episode_file(args):
    ...
    try:
        df_temp = pd.read_parquet(episode_file)

        episode = []
        for _, row in df_temp.iterrows():
            new_row = {'observation': {}, 'action': row.get('action', np.zeros(1))}
            for col, val in row.items():
                if col.startswith('observation.'):
                    key = col.split('.', 1)[1]
                    new_row['observation'][key] = val
                else:
                    new_row[col] = val

            if 'image' not in new_row['observation']:
                new_row['observation']['image'] = {}
        ...
        for row in episode:
            ...
            delta_action = apply_delta_transform_from_config(
                processed_state, raw_action, delta_mask
            )
        ...
        if compute_per_timestamp or compute_correlation:
            ...
            for i in range(len(states) - action_horizon + 1):
                ...
                future_actions = raw_actions[i:i + action_horizon]

                delta_chunk = np.zeros_like(future_actions)

                for t in range(action_horizon):
                    delta_chunk[t] = apply_delta_transform_from_config(
                        current_state,
                        future_actions[t],
                        delta_mask
                    )
            ...
            if len(action_chunks) > 0:
                ...
                n_samples = max(1, int(len(action_chunks) * sample_fraction))

                if sample_fraction < 1.0 and n_samples < len(action_chunks):
                    idx = rng.choice(len(action_chunks), size=n_samples, replace=False)
                    sampled = action_chunks[idx]
                else:
                    sampled = action_chunks

                if compute_per_timestamp:
                    per_timestamp_data = sampled

                if compute_correlation:
                    correlation_chunks = sampled
...
def main(
    ...
    # Find episode parquet files. Respect task_names so norm stats match
    # the same task subset used by training.
    data_root = Path(data_config.behavior_dataset_root)
    if data_config.task_names is not None:
        tasks_path = data_root / "meta" / "tasks.jsonl"
        task_name_to_index = {}
        with tasks_path.open("r") as f:
            for line in f:
                task = json.loads(line)
                task_name_to_index[task["task_name"]] = int(task["task_index"])

        missing_tasks = [task for task in data_config.task_names if task not in task_name_to_index]
        if missing_tasks:
            raise ValueError(f"Task names not found in {tasks_path}: {missing_tasks}")

        selected_task_indices = [task_name_to_index[task] for task in data_config.task_names]
        print(f"Filtering to configured tasks: {data_config.task_names}")
        print(f"Selected task indices: {selected_task_indices}")

        episode_files = []
        for task_index in selected_task_indices:
            task_pattern = f"data/task-{task_index:04d}/episode_*.parquet"
            print(f"Looking for episode files in: {data_root}/{task_pattern}")
            episode_files.extend(data_root.glob(task_pattern))
    else:
