...
import jax
# 1. 숨겨진 linear_util 경로를 강제로 찾아 연결합니다.
try:
    ...
    try:
        ...
        # 최악의 경우를 대비해 더 깊은 곳까지 뒤집니다.
        from jax._src import linear_util as lu

# 2. api_util.debug_info 속성 누락 방지
...
# 3. wrap_init 함수 이름표 불일치 방지
orig_wrap_init = lu.wrap_init
def patched_wrap_init(f, *args, **kwargs):
    kwargs.pop('debug_info', None) # 에러를 일으키는 범인을 제거합니다.
    return orig_wrap_init(f, *args, **kwargs)
lu.wrap_init = patched_wrap_init
