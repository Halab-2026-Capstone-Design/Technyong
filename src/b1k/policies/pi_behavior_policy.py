...
class PiBehaviorPolicy(Policy):
    ...
    def infer(self, obs: dict, *, noise: np.ndarray | None = None, initial_actions: np.ndarray | None = None) -> dict:
        ...
        if initial_actions is not None:
            ...
            ## 수정 : 여기서 쓰는 key 이름과 b1k_policy.py의 key 이름이 반드시 같아야 함.!!
            ...
            if "observation/egocentric_camera_semantic" in obs:
                 training_obs["observation/egocentric_camera_semantic"] = obs["observation/egocentric_camera_semantic"]
            elif "image" in obs and "base_0_semantic_rgb" in obs["image"]:
                training_obs["observation/egocentric_camera_semantic"] = obs["image"]["base_0_semantic_rgb"]
            ...
            if "observation/wrist_image_left_semantic" in obs:
                training_obs["observation/wrist_image_left_semantic"] = obs["observation/wrist_image_left_semantic"]
            elif "image" in obs and "left_wrist_0_semantic_rgb" in obs["image"]:
                training_obs["observation/wrist_image_left_semantic"] = obs["image"]["left_wrist_0_semantic_rgb"]
            ...
            if "observation/wrist_image_right_semantic" in obs:
                training_obs["observation/wrist_image_right_semantic"] = obs["observation/wrist_image_right_semantic"]
            elif "image" in obs and "right_wrist_0_semantic_rgb" in obs["image"]:
                training_obs["observation/wrist_image_right_semantic"] = obs["image"]["right_wrist_0_semantic_rgb"]
